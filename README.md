...
- [**🛠 주요 기술 상세**](#-주요-기술-상세)
  ...
  - 컨베이어 벨트 제어 — I2C 모터 + GPIO 센서
  - 예외 처리 및 초기화 — 커스텀 Exception 기반 공정 리셋
- [Troubleshooting(docekr pid1 문제)](#-트러블슈팅)
...
| **시스템 보안**     | 외부망(Cloud↔Edge) **mTLS 상호 인증** +  내부망(Edge↔Device) **ID/PW 인증** 적용, VPN 기반 폐쇄망 구축                            | `OpenSSL`, `Tailscale`, `Mosquitto ACL`      |
...
| **Edge 미들웨어**   | 로컬 MQTT 브로커 + 서버 MQTT 클라이언트 이중 구조의 게이트웨이 통신 모듈 개발. 팩토리 상태 머신 기반 공정 제어 로직 구현          | `Paho MQTT`, `Python`                        |
| **컨베이어 제어**   | I2C 통신 기반 DC모터/서보모터 제어, IR센서 인터럽트 감지, 삼색 LED 상태 표시. GPIO 콜백 기반 비동기 박스 감지                     | `RPi.GPIO`, `I2C`, `PWM`                     |
